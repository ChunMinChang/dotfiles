...
Everything here is static: `searchfox-cli` + `Read`. No builds, no tests.
...
| **Ambiguous** | Claim is coherent but admits multiple interpretations, or several different invariants would fit it. | `AskUserQuestion` to pin down which interpretation — quote the candidate invariants and let the user choose. Re-run gate. |
...
- **Ambiguous (no stateable invariant).** "H264 SPS parsing has an issue." → Which
  ...
  Termination? AskUserQuestion with the candidates.
