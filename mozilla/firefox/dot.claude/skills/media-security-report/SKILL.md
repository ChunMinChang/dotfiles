...
description: >
  Prepare, reproduce, and audit standalone upstream security reports for the
  third-party media libraries Firefox vendors — FFmpeg/ffvpx, libvpx, libaom,
  dav1d, the Xiph libraries, libpng, libjpeg-turbo, libsoundtouch, libsrtp,
  libwebrtc, libyuv, libwebp, nestegg, cubeb, mp4parse-rust and the in-tree
  components with no upstream. Works out which private channel the library
  actually has, reproduces on the vendored revision, traces the root cause with
  revision-pinned links, exports git-formatted patches, and drafts the
  submission without ever sending it. Triggers on: "/media-security-report",
  "report this to ffmpeg-security@ffmpeg.org", "prepare an upstream security
  report", "how do I report this <library> bug privately", "is this <library>
  crash security-relevant", "audit this security report".
argument-hint: "<evidence-path-or-claim> [--library <id>] [--firefox <path>] [--report-dir <path>] | --audit <report.md> --library <id>"
allowed-tools:
  - Bash
  - Read
  - Write
...
- `--firefox <path>` → checkout root. Default: walk up from cwd, since this
  skill is symlinked into each worktree's `.claude/skills/`.
...
- Anything still unresolved → ask with `AskUserQuestion`.

Create one task per step with `TaskCreate` on a fresh run. A report spans
several long builds; the task list is what tells a resumed session which steps
already have evidence on disk.
...
- **Never invent** reviewer or finder identities, CVEs, generators, stack
  ...
  `Not available`, or `Not applicable` with a reason, and use
  `AskUserQuestion` when a human attribution is required.
...
Delegate with `Agent` when a task is bounded, voluminous, or parallelisable.
...
as multiple `Agent` calls in a single message:
...
```sh
python3 .claude/skills/media-security-report/scripts/media_lib_facts.py --library <id>
```
...
- Read the supplied report and list every claimed manifestation, input, command
  ...
  attachments and logs directly with `Read`; do not rely on a summary when the
  source evidence is available.
- Identify the human reviewer(s) who will stand behind the verification and the
  human finder(s) to credit. Ask with `AskUserQuestion`; a bot is not a human
...
- Run the validator and fix every error; warnings require an explicit decision:
  ...
  ```sh
  python3 .claude/skills/media-security-report/scripts/validate_media_report.py \
      REPORT_DIR/<library>-security-report.md --library <id>
  ```

- Launch an independent audit subagent (`Agent`) that has not seen this run's
  reasoning. Give it the report path only, and have it write `research/audit.md`:
