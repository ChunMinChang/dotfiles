...
Sherlock launches bounded research with Codex `spawn_agent` subagents. Use
`worker` for deterministic artifact production and `explorer` for broader
search or hypothesis generation. When the runtime supports parallel subagent
calls, launch independent teams concurrently.
...
self-check passes, launch the three reviewers as independent `worker` subagents
in parallel when possible. Each writes a dedicated file under `<run_dir>/review/`
and returns a short verdict.
...
| **R (red-pen on root cause)** | `review/R.md` | Spawn an independent `worker` reviewer with `<abs path to bug-<id>-analysis.md>`. The reviewer challenges the root cause, the hypothesis-tree ranking, and the assumption labels. Verdict goes into the file. |
