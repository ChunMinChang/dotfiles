...
Sherlock launches research subagents in **one message containing multiple
`spawn_agent` calls**, each with `fork_turns: "none"`, so they run concurrently.
Every prompt is self-contained and passes absolute input, reference, repo, run-dir,
and output paths. No team inherits the main conversation.
...
Each prompt template below ends with an instruction to write to that file. Before
spawning, substitute every placeholder with an **absolute path or explicit value**
(not "the bug we're investigating"), including `<run_dir>` and
`<sherlock_skill_dir>`. Each prompt includes the framing *"return the requested
artifact only; do not draw conclusions about the root cause."*

---

## Wave 1 — intake teams

Before launching, ensure the main report exists: use a supplied `report-path`, or
fetch it once with `sherlock-config --fetch-bug`. Read only enough to prepare the
condensed identity/STR/expected/actual/stack input for H. Then launch Team B and
Team H in the **same message**. B digests that report and gathers the remaining
context; H brainstorms from the condensed input.
...
- Repo root: <repo-root>
- Main report input: <absolute report-path-or-directory>
- Sherlock config helper: <repo-root>/.codex/skills/sherlock/sherlock-config

Tasks:
1. If Main report input is already `<run_dir>/bug-<bug_id>-report`, use it. If it
   is another supplied file/directory, read it and do not re-fetch. Only when the
   input is explicitly `none`, run `<sherlock-config> --fetch-bug <bug_id> -o
   <run_dir>/bug-<bug_id>-report`.
...
- Run directory: <absolute run_dir>
- Sherlock skill directory: <absolute sherlock_skill_dir>
- Side: library: <name> @ <upstream_revision>
- Revision source: <exact path and field/type from upstream-libs.md, such as
  media/<lib>/moz.yaml revision, third_party/<lib>/moz.yaml revision, or Cargo.toml>
- Use upstream permalink construction (see
  <sherlock_skill_dir>/references/upstream-libs.md), e.g.
...
2. Read the introducing commit message in full. Follow the linked Bugzilla bug
   (use <repo-root>/.codex/skills/sherlock/sherlock-config --fetch-bug).
...
Follow <sherlock_skill_dir>/references/spec-check.md. Return findings only — do
NOT pick the verdict.
...
Searchfox revision: <$SHERLOCK_REV>
Reference: <sherlock_skill_dir>/references/test-frameworks.md
  (decision tree + FuzzingFunctions map).
...
- **T1** (locate the upstream repo) needs interactive user input — ask the user directly.
...
| Reviewer | File | Job |
|---|---|---|
| **L (links / citations)** | `review/L.md` | Open local source targets or remote permalinks with the appropriate current tool. Confirm the cited file/line still says what the doc claims. Replace any trunk URL with a `$SHERLOCK_REV` link. Report pass/fail + fix-up diffs. |
| **T (test re-runner)** | `review/T.md` | Re-read every applicable `firefox/debug/bug-<id>-<proof-id>-test-run.log`. Re-verify by applying exported patches onto a disposable worktree at `$SHERLOCK_BASE`, never onto the working branch. Confirm each proof FAILs **on the bug**, not setup. Reject landing-branch injection tests. Report pass/fail. |
| **R (red-pen on root cause)** | `review/R.md` + `review/bug-<id>-rootcause-review.md` | The main agent performs Red Pen pre-flight and spawns the critic directly at depth one with `fork_turns: "none"`; no wrapper agent may spawn another child. The critic writes the full explicit-path review. The main agent writes `review/R.md` from the returned verdict/headline/iteration plus a pointer to the exact full-review path. |
...
After Wave 2, set the synthesis row `in-progress`, read every **applicable** team
file whose row is completed (not subagent transcripts), and write
`<run_dir>/teams/synthesis.md`. Team L/X may be skipped; Team D is replaced by
the same required artifact path in third-party runs:
...
- Repo root: <repo-root>
- Searchfox revision: <$SHERLOCK_REV>
- Method: <absolute sherlock_skill_dir>/references/first-principles.md, section Q4
...
- Repo root: <repo-root>
- Searchfox revision: <$SHERLOCK_REV>
- Method: <absolute sherlock_skill_dir>/references/first-principles.md, section Q5
...
- Effort, risk, performance, and uplift-friendliness are descriptive comparisons
  only. Do not assign weights, calculate scores, rank options, or recommend one;
  Decide performs those judgments after the option set is agreed.
...
- Investigation plan / `update_plan`.
