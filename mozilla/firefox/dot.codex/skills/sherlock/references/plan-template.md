...
- **Firefox branch base (`SHERLOCK_BASE`):** `{base_sha}` — `not-created` until
  `Diagnose.6`, then the local HEAD from which the branch was cut. This is **not**
  the same as the Searchfox revision above, which may
  be an older indexed commit. The `Implement.3` reorder and branch recovery need this.
- **Firefox working-branch tip:** `{tip_sha}` — `not-created` until row 8, then
  updated only for commits on `sherlock/bug-{bug_id}`. Debug and library commits
  never overwrite it.
- **Firefox evidence refs:** {one `branch @ tip` per hypothesis/debug branch, or `none`}
...
- **Library ref tips:** test `{library_test_tip}` · debug `{library_debug_tip}` ·
  fix `{library_fix_tip}` — `not-created` until each ref exists
...
Sherlock reads this file and finds the first **non-terminal** row in table order.
Only `completed` and `skipped` are terminal. `pending` and `in-progress` resume
work; `blocked-on-user` re-presents the gate; `blocked-external` checks the named
dependency and either remains POSTPONED or reopens implementation verification.
Completed rows are trusted only while their inputs and recorded refs are intact.
For aggregate parents with dynamic children (16, 29, 42), an `in-progress`
parent resumes at its first non-terminal child; completed children are not rerun.
...
`blocked-external` is for work that cannot proceed until something outside this
run lands (a blocking bug, an upstream release). It pauses the row and makes
Consolidate close the current session as **POSTPONED**, not FINISHED. On resume it
is the first non-terminal row and must be checked before later work. Do not mark a
genuine blocker `skipped` just to close the table out.
...
| 6   | Diagnose    | `Diagnose.4` investigation plan (update_plan)             | pending  | harness plan + Notes                                         |
...
| 14  | Diagnose    | `Diagnose.9` third-party decision (T1/T2/T3 diagnostic)     | pending  | `<library>/debug/` logs + scope verdict in Notes             |
...
| 17.25 | Diagnose  | Final two-tier claim audit                                  | pending  | analysis-doc Verified Claims / Assumptions                   |
...
| 43  | Implement   | `Implement.4` verify FAIL→PASS or documented alternative evidence | pending | `firefox/fix/bug-{bug_id}-verify-run.log`                 |
...
| 46  | Consolidate | Close run as FINISHED or POSTPONED                          | pending  | `Run status` line above                                      |
...
| 16.1 | Diagnose | Validate H1: OOB read in vorbis window function | pending | `firefox/debug/<NN>-test-h1-*.patch`, `firefox/debug/bug-{bug_id}-h1-test-run.log` (`firefox/fix/` copy only when allowed) |

Set parent row 16 `in-progress` before processing children and `completed` only
after every child is `completed` or `skipped`. If no hypothesis can support a
stable proof test, mark row 16 `skipped` with the alternative evidence in Notes.

**Third-party Branch rows.** Row 14 (T1/T2/T3) is an internal diagnostic decision
point, not one of the five user phase gates. When T3 resolves
...
- **Branch A (library bug):**
  - `14.1 | Diagnose | A1 library investigation + design intention | pending | teams/team-l-code-trace-library.md, teams/team-d-design-archaeology.md`
  - `14.2 | Diagnose | A2 Firefox regression proof | pending | firefox/debug/<NN>-test-a2-*.patch, firefox/debug/bug-{bug_id}-a2-test-run.log`
  - `14.3 | Diagnose | A3 upstream report | pending | <library>/bug-{bug_id}-upstream-<lib>.md`
- **Branch B (Firefox integration):**
  - `14.1 | Diagnose | B1 pivot trace + design intention + proof | pending | teams/team-d-design-archaeology.md, firefox/debug/<NN>-test-b1-*.patch, firefox/debug/bug-{bug_id}-b1-test-run.log`
- **Branch C (split scope):** the two-layer rows (library + Firefox traces,
  tests, and upstream report):
  - `14.1 | Diagnose | C1 dual-layer investigation + design intention | pending | teams/team-{c,l,d}-*.md`
  - `14.2 | Diagnose | C2 library + Firefox proof evidence | pending | <library>/debug/…, firefox/debug/…`
  - `14.3 | Diagnose | C3 upstream report (skip when no library report is warranted) | pending | <library>/bug-{bug_id}-upstream-<lib>.md`

**Deferred fix work goes under row 42, not row 14.** At the same time T3 records
the scope verdict, append exactly one matching Implement child row in the row-42
position of the table:

- `42.1 | Implement | A4 library fix + vendor + re-verify | pending | <library>/fix/...`
- `42.1 | Implement | B2 Firefox integration fix + re-verify | pending | firefox/fix/...`
- `42.1 | Implement | C4 library/Firefox fixes + vendor + re-verify | pending | <library>/fix/..., firefox/fix/...`

Putting them under 14 would break resume. The table is scanned for the *first*
non-terminal row, so a `pending` row 14.4 sitting through Diagnosis Review, four
...
separate run; record the exact returned output path and keep every review. The
first collision uses `-1`, then `-2`, and so on. Append a row per re-run under row 38:

| 38.1 | Decide | red-pen run 2 (after option E appended) | pending | `bug-{bug_id}-review-1.md` |

**Follow-up and blocking rows.** `Implement.6` appends one row per issue found
during implementation:

| 44.1 | Implement | Blocking: {issue} — must land before this bug | blocked-external | Notes + analysis-doc Related Context |
...
The Design↔Decide loop is expected. Reopening any row invalidates all dependent
completed rows; record the reset set in Notes. When Decide sends work back:
...
- **Back to Design** — set rows 31/32/33 to `in-progress`, **append** a new
  ...
  Reset rows 34–40 and 42–46, and append a new 38.x review row.
- **Back to Reframe** — set row 27 `in-progress` and append a new principle;
  reopen row 22 too when the framing changed, then reset rows 28–46.
- **Back to Diagnose** (reviewer `reject`, or the proof test fails to flip) —
  set the offending Diagnose row `in-progress`, reset its dependent Diagnose
  rows through 21, then reset rows 22–46 and re-run forward.
