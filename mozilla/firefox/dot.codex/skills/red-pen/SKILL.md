...
metadata:
  short-description: Independent fix review
---
...
1. **Token resolves to an existing file** (try as-given, then relative to
   ...
   - any other extension or extensionless → ask the user directly. Do not guess.
...
ask the user directly. Never silently pick a winner.
...
If after parsing there is no analysis source, ask the user directly. Do not
guess.
...
directory at `~/.codex/red-pen-scratch/<run-id>/`, where `<run-id>` is
...
- Never modify the analysis doc or solutions doc, nor any user-supplied
  ...
  transport docs in `~/.codex/red-pen-scratch/<run-id>/`; everything
  else is read-only.
