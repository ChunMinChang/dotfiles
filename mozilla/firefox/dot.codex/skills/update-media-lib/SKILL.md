...
metadata:
  short-description: Update media libraries
---
...
**IMPORTANT:** Before starting any work, determine what kind of update the user needs. If the user's request clearly indicates one type (e.g., they provide a commit URL to cherry-pick, or say "update to latest"), skip the question and proceed directly. Otherwise, ask the user directly:
...
3. **Fetch the upstream diff** - Use Codex web access or `curl` to get the commit diff (e.g., from Googlesource `?format=TEXT` for base64-encoded diffs, or from GitHub)
...
5. **Ask for Bugzilla number** - ask the user directly for an optional bug number
...
1. Find the Firefox release date (use web search when needed)
...
ALWAYS ask the user if they have a Bugzilla bug number for this update. Ask it
alongside the commit preference question.
...
Ask for both the commit preference and optional bug number before proceeding.
